<x-app-layout>
    ...
    @include('HousekeepingAdmin.housekeepingWorkOrders.housekeeping_work_orders_modal')
    <div class="pt-6">
        <div class="panel">
            <form action="{{ url('HousekeepingAdmin/submit-work-order/id='.$h_request_id->h3_request_code) }} "
                method="POST">
                @csrf
                <!-- Invoice -->
                <div class="max-w-full px-4 sm:px-6 lg:px-8 my-4 sm:my-10">
                    ...
                    <div
                        ...
                        <div>
                            <h2 class="text-2xl  text-gray-800 dark:text-gray-200">Housekeeping Work Order</h2>
                        </div>
                    </div>
                    <!-- End Grid -->
                            ...
                            <div class="grid space-y-3">
                                <dl class="grid sm:flex gap-x-3 text-sm items-center">
                                    ...
                                    <dd class="text-gray-800 dark:text-gray-200">
                                        <span
                                            ...
                                            {{$h_request_id->h3_request_code}}</span>
                                    </dd>
                                </dl>
                                ...
                                <dl class="grid sm:flex gap-x-3 text-sm">
                                    ...
                                    <dd class="font-medium text-gray-800 dark:text-gray-200">
                                        <span class="block font-semibold">{{$h_request_id->h3_room_no}}</span>
                                        <input type="hidden" id="room_no" name="room_no" readonly
                                            value="{{$h_request_id->h3_room_no}}"
                                            class="ml-2 w-[50vh] rounded-lg border-[1px] border-stroke bg-transparent px-5 py-3 text-sm text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary" />
                                    </dd>
                                </dl>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="grid space-y-3">
                                <dl class="grid sm:flex gap-x-3 text-sm items-center">
                                    ...
                                    @endphp
                                    @if($h_request_id->status == 'Work Order Created')
                                    <div class="py-2">
                                    ...
                                    @elseif(\Carbon\Carbon::parse($h_request_id->created_at)->toDateString() != $today_date)
                                    <div class=" py-2">
                                ...
                                <dl class="grid sm:flex gap-x-3 text-sm">
                                    ...
                                    <dd class="font-medium text-gray-800 dark:text-gray-200">
                                        <span
                                            ...
                                            $h_request_id->h3_time_issue);

                                            $formattedTime = $time->format('h:i A');
                                            @endphp
                                            {{$formattedTime}}
                                        </span>
                                        <input type="hidden" id="time" name="scheduled_time" readonly
                                            value="{{$formattedTime}}"
                                ...
                                <dl class="grid sm:flex gap-x-3 text-sm items-center">
                                    ...
                                    <dd class="font-medium text-gray-800 dark:text-gray-200">
                                        <div x-data="{ isOptionSelected: false }"
                                            class="relative dark:bg-form-input">
                                            <select name="housekeeper_name"
                                            class="form-select text-white-dark">
                                                <option selected disabled>Select Housekeeper...</option>
                                            ...
                                            @if ($errors->has('housekeeper_name'))
                                            <span class="text-red-500 text-[13px]">*{{
                                                $errors->first('housekeeper_name')
                                                }}</span>
                                            @endif
                                        </div>
                                    </dd>
                                    ...
                                    <div class="border overflow-hidden dark:border-gray-700">
                                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                                @foreach(json_decode($h_request_id->h3_housekeeping_request) as $request)
                                                <tr
                                                    class="bg-white border-b text-sm dark:bg-gray-900 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                                    <td scope="row"
                                ...
                                <div class="p-1.5 min-w-full inline-block align-middle">
                                    <div class="border overflow-hidden dark:border-gray-700">
                                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                                @foreach(json_decode($h_request_id->h3_items_services) as $request)
                                                <tr
                    ...
                    <div class="mt-3">
                        <textarea id="ctnTextarea" rows="3" class="form-textarea" value="{{$h_request_id->h3_additional_request}}" name="additional_requests">{{$h_request_id->h3_additional_request}}</textarea>
                    </div>
                </div>
                ...
                <div class="flex justify-between items-center mt-6 mx-8">
                    <h4 class="relative top-3 text-xs font-semibold uppercase text-gray-800 dark:text-gray-200">
                        ITEMS NEEDED</h4>
                    <button data-hs-overlay="#addCart"
                            ...
                            <div class="border rounded-lg overflow-hidden dark:border-gray-700">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    ...
                                    <tbody class=" divide-gray-200 dark:divide-gray-700">
                                        @forelse($work_order_cart as $cart)
                                        <tr>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">
                                                {{$cart->inventoryItem->h3_item_name}}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{$cart->inventoryItem->h3_category_name}}</td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                {{$cart->h3_quantity}}
                                            </td>
                                            <td
                                                class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">
                                                <a href="{{ url('HousekeepingAdmin/delete-item-cart/id='.$cart->id)}}"
                                                    class="text-red-700 ">
