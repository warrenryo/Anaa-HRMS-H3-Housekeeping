<x-app-layout>
    @section('title', 'Housekeeping Requests')
        ...
        <div class="panel">
            <div class="flex items-center justify-between mb-5">
                <form action="" method="GET" class="flex items-center">
                    <dd class="font-medium text-gray-800 dark:text-gray-200 mr-2">
                        <div x-data="{ isOptionSelected: false }" class="relative dark:bg-form-input">
                            <select name="filter_status" class="form-select text-white-dark">
                                <option selected disabled>Select Status</option>
                                <option class="text-black dark:text-white-dark" {{Request::get('filter_status')=='Pending' ? 'selected'
                                    :''}} value="Pending">Pending
                                </option>
                                <option class="text-black dark:text-white-dark" {{Request::get('filter_status')=='Work Order Created'
                                    ? 'selected' :''}} value=" Work Order Created">Task Created</option>
                            </select>
                        </div>
                    </dd>
                ...
                <div class="text-center">
                    <button type="button" class="btn btn-danger" data-hs-overlay="#hs-danger-alert">
                        Delete All Exceeded
                    </button>
                </div>
                                ...
                                <div class="flex gap-x-4 md:gap-x-7">
                                    ...
                                    <div class="grow">
                                        <h3 class="mb-2 text-xl font-bold text-gray-800 dark:text-gray-200">
                                            Delete all Exceeded Requests
                                        </h3>
                                        <p class="text-gray-500">
                                            Permanently remove Exceeded Status and all of its contents from the ANAA HOTEL HOUSEKEEPING. This action is not reversible, so please continue with caution.
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="mb-5">
                <div class="table-responsive">
                    <table class="table-hover">
                        <thead>
                            <tr>
                                <th class="text-xs">REQUEST CODE</th>
                                <th class="text-xs">ROOM NUMBER</th>
                                <th class="text-xs">TIME ISSUE</th>
                        ...
                        <tbody>
                            @foreach($h_request as $request)
                            <tr>
                                <td class="whitespace-nowrap">{{$request->h3_request_code}}</td>
                                <td>{{$request->h3_room_no}}</td>
                                <td>
                                    @php
                                    // Convert the string to a Carbon object
                                    $carbonTime = Carbon\Carbon::createFromFormat('H:i',
                                    $request->h3_time_issue);
                                ...
                                <td>
                                    ...
                                    @endphp
                                    @if($request->status == 'Work Order Created')
                                    <div class="py-2">
                                    ...
                                    @elseif(\Carbon\Carbon::parse($request->created_at)->toDateString() != $today_date)
                                    <div class=" py-2">
                                ...
                                <td>
                                    @include('HousekeepingAdmin.housekeepingRequest.housekeepingRequestModal')
                                    <div class="hs-tooltip inline-block [--trigger:hover] mr-2">
                                        <button type="button" data-hs-overlay="#edit-staff{{$request->id}}" class="hs-tooltip-toggle flex justify-center items-center h-10 w-10 text-sm font-semibold rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">

                                            <svg class="hover:animate-rotate dark:text-white " width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <a href="{{url('HousekeepingAdmin/create-work-orders/request-code='.$request->h3_request_code)}}" class="hs-tooltip inline-block [--trigger:hover]">
                                        <button type="button" class="hs-tooltip-toggle flex justify-center items-center h-10 w-10 text-sm font-semibold rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                                            <svg class="hover:animate-rotate hover:text-primary" width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
