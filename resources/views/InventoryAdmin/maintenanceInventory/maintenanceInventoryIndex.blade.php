            ...
            <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-3.5rem)] flex items-center justify-center">
                <div class="flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto dark:bg-gray-800 dark:border-gray-700 dark:shadow-slate-700/[.7]">
                    ...
                    <div class="p-4 overflow-y-auto">
                        <form action="{{ url('inventory-admin/submit-maintenance-inventory') }}" method="POST">
                            @csrf
                            <div>
                                <label for="ctnSelect1">Category</label>
                                <select id="ctnSelect1" name="m_category" class="form-select text-white-dark" required>
                                    <option>Select Maintenance Category</option>
                                    @foreach($m_category as $category)
                                    <option>{{$category->h3_m_category}}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mt-4">
                                <label for="ctnEmail">Item Name</label>
                                <input id="ctnEmail" name="item_name" type="text" placeholder="Enter name" class="form-input" required />
                            </div>
            ...
            <table class="table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="whitespace-nowrap">
                            @if($items->h3_quantity > 0 && $items->h3_quantity <= 5) <span class="inline-flex rounded-full bg-warning bg-opacity-10 px-3 py-1 text-sm font-medium text-warning"">{{$items->h3_quantity}}</span>
                        ...
                        <td class="flex ">
                            @include('InventoryAdmin.maintenanceInventory.maintenanceInventoryModal')
                            <div class="hs-tooltip inline-block [--trigger:hover] mr-2">
                                <button type="button" data-hs-overlay="#edit-items{{$items->id}}" class="hs-tooltip-toggle flex justify-center items-center h-10 w-10 text-sm font-semibold rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none dark:bg-slate-900 dark:border-gray-700 dark:text-white dark:hover:bg-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
