        ...
        <div class="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-[#0e1726]">
            <div class="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                <a href="/" class="main-logo flex items-center shrink-0">
                    <img class="w-8 ltr:-ml-1 rtl:-mr-1 inline" src="{{ url('img/anaa_logs.png') }}" alt="image" />
                ...
                <div class="dropdown" x-data="dropdown" @click.outside="open = false">
                    ...
                    <ul x-cloak x-show="open" x-transition x-transition.duration.300ms class="ltr:-right-2 rtl:-left-2 top-11 !py-0 text-dark dark:text-white-dark w-[300px] sm:w-[350px] divide-y dark:divide-white/10">
                        ...
                        <div class="overflow-y-auto h-[50vh] ">
                            ...
                            <li class=" dark:text-white-light/90 border-b ">
                                <div class="flex items-center px-4 py-2 group" @click.self="toggle">
                                    ...
                                    <a href="{{ url('markUnread'.$notification->id) }}" class="ltr:pl-3 rtl:pr-3 flex flex-auto">
                                        <div class="ltr:pr-3 rtl:pl-3">
                                            <h6><strong class="text-sm mr-1">#{{$notification->data['code']}} -
                                                    {{$notification->data['room_no']}}</strong> has requested
                                                housekeeping scheduled at <strong>{{
                                                    \Carbon\Carbon::parse($notification->data['time_issue'])->format('g:i
                                                    A')}}</strong></h6>
                                            <span class="text-xs block font-normal dark:text-gray-500">{{$notification->created_at->diffForHumans()}}
                                                <span class="inline-flex text-[10px] rounded-full bg-success bg-opacity-10 px-3 py-1 text-success dark:bg-green-200">New</span></span>
                                        </div>
                                    ...
                                    @elseif($notification->type == 'App\Notifications\CompletedTaskNotification')
                                    <a href="{{ url('markUnread'.$notification->id) }}" class="ltr:pl-3 rtl:pr-3 flex flex-auto">
                                        <div class="ltr:pr-3 rtl:pl-3">
                                        <h6><span class="text-sm mr-1"><strong>{{$notification->data['housekeeper_name']}}</strong> has completed the housekeeping task on <strong>Room {{$notification->data['room_no']}}</strong>
                                                    </span> at <strong>{{
                                                    \Carbon\Carbon::parse($notification->data['updated'])->format('g:i
                                                    A')}}</strong></h6>
                                            <span class="text-xs block font-normal dark:text-gray-500">{{$notification->created_at->diffForHumans()}}
                                                <span class="inline-flex text-[10px] rounded-full bg-success bg-opacity-10 px-3 py-1 text-success dark:bg-green-200">New</span></span>
                                        </div>
                                        ...
                                        <div class="ltr:pr-3 rtl:pl-3">
                                            <h6><strong class="text-sm mr-1">#{{$notification->data['code']}} -
                                                ...
                                                housekeeping scheduled at <strong>{{
                                                    \Carbon\Carbon::parse($notification->data['time_issue'])->format('g:i
                                                    A')}}</strong></h6>
                                            <span class="flex items-center gap-2 text-xs block font-normal dark:text-gray-500">{{$notification->created_at->diffForHumans()}}
                                                <span class="inline-flex text-[10px] rounded-full bg-gray-500 bg-opacity-10 px-3 py-1 text-success dark:text-white dark:bg-gray-500"><i class="lni lni-checkmark"></i></span>
                                        </div>
                                        <button type="button" class="ltr:ml-auto rtl:mr-auto text-neutral-300 hover:text-danger opacity-0 group-hover:opacity-100" @click="removeNotification(notification.id)">
                                            <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <circle opacity="0.5" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="1.5" />
